...
struct HeartDataChartView: View {
        ...
        VStack(alignment: .leading, spacing: 5) {
            ...
            GroupBox(metricTitle) {
                Chart {
                    ...
                    ForEach(metricData, id: \.date) {
                        data in
                        
                        LineMark(x: .value("day", data.date, unit: .day),
                                 y: .value(yValueTitle, data.value)
                        )
    ...
    var annotationView: some View {
        VStack(alignment: .leading) {
            ...
            Text("\(selectedHealthValue?.value ?? 0, format: .number.precision(.fractionLength(0))) \(unitName)")
                .fontWeight(.semibold)
                .foregroundStyle(.red)
        }
        .padding(12)
...
#Preview {
    HeartDataChartView(metricTitle: "Resting Heart Rate", yValueTitle: "RHR", unitName: "bpm", metricData: MockData.heartRate)
}
